...
Squid Digest is a Django-based automation tool that generates AI-powered daily news digests. It fetches trending crypto/tech news from [Leviathan News API](https://api.leviathannews.xyz), processes headlines through Perplexity AI (or OpenAI) to generate intelligent summaries, and publishes the digest to Ghost CMS.
...
**Always stage and commit all modified files before completing a task.** After making code changes:
1. Stage all modified files: `git add <file>` or `git add .` for all changes
2. Ensure `git status` shows a clean working directory (all changes staged)
3. **Create the git commit** with an appropriate commit message
4. The user handles git push - do NOT push commits

**Important:** You are responsible for git commits. The user handles git pushes. Always commit your changes before considering a task complete.
