...
class TestFooterAndDisclaimer:
    ...
    def test_footer_after_portfolio_section(self):
        """Test that footer and disclaimer are added after portfolio section."""
        ...
        # Portfolio section (backtest_section variable) should be appended
        assert 'backtest_section' in script_content, "Portfolio section variable should exist"
        assert '+ backtest_section' in script_content, "Portfolio section should be appended"
        assert 'Disclaimer:' in script_content, "Disclaimer should be present"

        # Find the order: portfolio section should come before footer
        portfolio_idx = script_content.find('+ backtest_section')
        disclaimer_idx = script_content.find('Disclaimer:')

        assert portfolio_idx != -1, "Portfolio append should be found"
        assert disclaimer_idx != -1, "Disclaimer should be present"
        # Verify order: portfolio section comes before disclaimer (which is part of footer)
        assert portfolio_idx < disclaimer_idx, f"Portfolio section (at {portfolio_idx}) should be added before disclaimer/footer (at {disclaimer_idx})"
